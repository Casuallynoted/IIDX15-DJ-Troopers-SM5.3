...
--[[ 	InitCommand=cmd(x,SCREEN_CENTER_X+160;y,SCREEN_CENTER_Y+91);
	...
	SetCommand=function(self)
		...
		self:PositionItems();
		self:SetTransformFromHeight(20);
...
-- 		self:SetDestinationItem( self:GetNumItems()-2 );
	...
	Display = Def.ActorFrame { 
		InitCommand=cmd(setsize,270,20);
...
-- 					self:diffuse( CustomDifficultyToColor(params.Difficulty) );
				...
				(cmd(finishtweening;diffusealpha,0;sleep,0.05*params.Number;linear,0.1;diffusealpha,1;linear,0.05;glow,color("1,1,1,0.5");decelerate,0.05;glow,color("1,1,1,0")))(self);
			end;
		};

		Def.TextBanner {
			InitCommand=cmd(x,-65;y,0;Load,"TextBanner";SetFromString,"", "", "", "", "", "");
			SetSongCommand=function(self, params)
				if params.Song then
...
-- 					self:glow("1,1,1,0");
				...
				(cmd(finishtweening;zoomy,0;sleep,0.05*params.Number;linear,0.1;zoomy,.9;linear,0.05;zoomx,.9;decelerate,0.05;zoom,.8))(self);
			end;
		};
...
--[[ 		LoadFont("CourseEntryDisplay","number") .. {
 		...
 		LoadFont("StepsDisplay","meter") .. {
			Text="0";
			InitCommand=cmd(x,92;y,-5;zoom,0.75;shadowlength,1);
			SetSongCommand=function(self, params)
				if params.PlayerNumber ~= GAMESTATE:GetMasterPlayerNumber() then return end
				self:settext( params.Meter );
				self:diffuse( CustomDifficultyToColor(params.Difficulty) );
				(cmd(finishtweening;zoomy,0;sleep,0.05*params.Number;linear,0.1;zoomy,.6;linear,0.05;zoomx,.6;decelerate,0.05;zoom,.5))(self);
			end;
		}; 
